...
def _fetch_config(service):
    base_waiter_path = str(Path(__file__).parent).replace("tests", "airflow")
    with open(f"{base_waiter_path}/{service}.json") as config_file:
...
class TestServiceWaiters:
    @pytest.mark.parametrize("service, hook", [("eks", EksHook)])
    def test_service_waiters(self, service, hook):
        expected_waiters = _fetch_config(service)["waiters"]
