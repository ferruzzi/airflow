...
from airflow.utils.helpers import render_template_to_string, prune_dict
...
class TaskInstance(Base, LoggingMixin):
    ...
    def __init__(
        ...
        self.run_id = run_id
        self.stat_tags = prune_dict({"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id})
    ...
    def _run_raw_task(
        ...
        Stats.incr(f"ti.start.{self.task.dag_id}.{self.task.task_id}", tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id})
        # Initialize final state counters at zero
        for state in State.task_states:
            Stats.incr(
                f"ti.finish.{self.task.dag_id}.{self.task.task_id}.{state}", count=0, tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id}
            )
        ...
        try:
            ...
            Stats.incr(f"ti.finish.{self.dag_id}.{self.task_id}.{self.state}", tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id})
    ...
    def _execute_task_with_callbacks(self, context, test_mode=False):
        ...
        with Stats.timer(f"dag.{self.task.dag_id}.{self.task.task_id}.duration", tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id}):
        ...
        Stats.incr(f"operator_successes_{self.task.task_type}", tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id})
        Stats.incr("ti_successes", tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id})
    ...
    def handle_failure(
        ...
        Stats.incr(f"operator_failures_{self.operator}", tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id})
        Stats.incr("ti_failures", tags={"dag_id": self.dag_id, "run_id": str(self.run_id), "task_id": self.task_id})
