...
class DagRun(Base, LoggingMixin):
    ...
    def update_state(
        ...
        with Stats.timer(f"dagrun.dependency-check.{self.dag_id}", tags={"dag_id": self.dag_id, "run_type": self.run_type}):
            dag = self.get_dag()
    ...
    def _emit_duration_stats_for_finished_state(self):
        ...
        if self.state == State.SUCCESS:
            Stats.timing(f"dagrun.duration.success.{self.dag_id}", duration, tags={"dag_id": self.dag_id, "run_type": self.run_type})
            Stats.timing("dagrun.duration.success", duration, tags={"dag_id": self.dag_id, "run_type": self.run_type})
        elif self.state == State.FAILED:
            Stats.timing(f"dagrun.duration.failed.{self.dag_id}", duration, tags={"dag_id": self.dag_id, "run_type": self.run_type})
            Stats.timing("dagrun.duration.failed", duration, tags={"dag_id": self.dag_id, "run_type": self.run_type})
