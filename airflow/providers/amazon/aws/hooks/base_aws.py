...
class AwsGenericHook(BaseHook, Generic[BaseAwsConnection]):
    ...
    def get_waiter(self, waiter_name: str) -> Waiter:
        ...
        """
        official_waiters = self.conn.waiter_names
        custom_waiters = [waiter for waiter in self.list_waiters() if waiter not in official_waiters]
        if self.waiter_path and (waiter_name in custom_waiters):
    ...
    def list_waiters(self) -> list[str]:
        ...
        official_waiter_names = self.conn.waiter_names
        custom_waiter_names = []

        if self.waiter_path:
            with open(self.waiter_path) as config_file:
                model_config = json.load(config_file)
            custom_waiter_names = WaiterModel(model_config).waiter_names

        return [*official_waiter_names, *custom_waiter_names]
